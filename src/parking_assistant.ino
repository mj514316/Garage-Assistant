...
#define SENSOR "SR04"                       // SR04 or TfMini. For TfMini, SERIAL_DEBUG must be set to 0 to read distance
const int SR04_trigPin = D1;                // for SR04 config
const int SR04_echoPin = D2;                // for SR04 config
#define SR04_DEBUG 1                        // because SR04 doesn't use serial pins, you can monitor it's measurments directly
...
void setup_SR04(){
  ...
  pinMode(SR04_trigPin, OUTPUT); // Sets the SR04_trigPin as an Output
  pinMode(SR04_echoPin, INPUT); // Sets the SR04_echoPin as an Input
}
...
float read_distance(){
  ...
  delayMicroseconds(2);
  // Sets the SR04_trigPin on HIGH state for 10 micro seconds
  digitalWrite(SR04_trigPin, HIGH);
  delayMicroseconds(10);
  digitalWrite(SR04_trigPin, LOW);

  // Reads the SR04_echoPin, returns the sound wave travel time in microseconds
  duration_SR04 = pulseIn(SR04_echoPin, HIGH);

  // Calculate the distance
  return duration_SR04 * 0.034/2 * 0.393701; //Convert from CM to In
}
    ...
    if (SPIFFS.exists("/config.json")) {
      ...
      if (configFile) {
        ...
        if (json.success()) {
          ...
          strcpy(start_mils, json["start_mils"] | "1828");
          strcpy(park_mils, json["park_mils"] | "609");
          strcpy(backup_mils, json["backup_mils"]| "475");
          #if defined(SERIAL_DEBUG) && (SERIAL_DEBUG == 1)
            Serial.println("\nread_park section");
          #endif
          strcpy(color_standby, json["color_standby"]| "Green");
          strcpy(color_wake, json["color_wake"]| "Yellow");
          strcpy(color_active, json["color_active"]| "Red");
          strcpy(color_parked, json["color_parked"]| "Red");
          strcpy(color_backup, json["color_backup"]| "Blue");
          #if defined(SERIAL_DEBUG) && (SERIAL_DEBUG == 1)
            Serial.println("\nread_color section");
          #endif
          ...
          strcpy(mqtt_port, json["mqtt_port"] | "N/A");
          strcpy(mqtt_tele_period, json["mqtt_tele_period"]| "100");
          strcpy(mqtt_user, json["mqtt_user"]| "xxx");
          strcpy(mqtt_pw, json["mqtt_pw"]| "XXXX");
          #if defined(SERIAL_DEBUG) && (SERIAL_DEBUG == 1)
            Serial.println("\nread mqtt");
          #endif
...
void loop() {
  ...
  if(SENSOR =="TfMini"){
    ...
    tf_dist = read_distance() * 10;
    #if defined(SERIAL_DEBUG) && (SERIAL_DEBUG == 1) && (defined(SERIAL_DEBUG) && (SR04_DEBUG ==1))
      Serial.print("Distance Measured by SR04:");
      Serial.println(tf_dist);
    #endif
